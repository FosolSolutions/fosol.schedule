  ...
  static class ApiHelper
  {
    ...
    public static object GetModelExample(Type type)
    {
      ...
      foreach (var prop in type.GetProperties(BindingFlags.Instance | BindingFlags.Public))
      {
        ...
        if (info.IsArray)
        {
          var agtype = Assembly.GetAssembly(typeof(Models.Read.Calendar)).GetType($"Fosol.Schedule.Models.{info.Type}") ?? Assembly.GetAssembly(typeof(Entities.Calendar)).GetType($"Fosol.Schedule.Entities.{info.Type}") ?? throw new InvalidOperationException($"Unable to create a default property for type '{prop.Name}'='{info.Type}'.");

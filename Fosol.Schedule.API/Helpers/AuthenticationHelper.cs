...
namespace Fosol.Schedule.API.Helpers
{
	...
	public static class AuthenticationHelper
	{
		...
		public static void AuthorizeOauthUser(this IDataSource datasource, ClaimsPrincipal principal)
		{
			...
			if (userId == 0)
			{
				var create = new Models.Create.User()
				{
					Key = Guid.NewGuid(),
					Info = new Models.Create.UserInfo()
					{
					...
					Email = email,
					OauthAccounts = new List<Models.Create.OauthAccount>(new[]
					{
						new Models.Create.OauthAccount()
						{
				...
				datasource.Users.Add(create);
				datasource.CommitTransaction();
			}

			var user = datasource.Users.Get(userId);
			var identity = principal.Identity as ClaimsIdentity;

...
namespace Fosol.Schedule.DAL.Services
{
  ...
  /// </summary>
  public sealed class UserService : UpdatableService<Entities.User, Models.Create.User, Models.Read.User, Models.Update.User, Models.Delete.User>, IUserService
  {
    #region Variables
    #endregion
    ...
    /// <returns></returns>
    public Models.Read.User Get(Guid key)
    {
      return this.Source.Map<Models.Read.User>(this.Find((set) => set.Include(u => u.Info).SingleOrDefault(u => u.Key == key)));
    }
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public Models.Read.User Get(int id)
    {
      return this.Source.Map<Models.Read.User>(this.Find((set) => set.Include(u => u.Info).SingleOrDefault(u => u.Id == id)));
    }
    ...
    public IEnumerable<Claim> GetClaims(int userId)
    {
      ...
      var claims = new List<Claim>(new[]
      {
                ...
                new Claim(ClaimTypes.Gender, $"{user.Info.Gender}", typeof(Models.Gender).FullName, "CoEvent"),
    ...
    public override Models.Update.User Add(Models.Create.User model)
    {
      var entity = this.Source.Map<Entities.User>(model);

...
namespace Fosol.Schedule.DAL.Services
{
  ...
  /// </summary>
  public sealed class OpeningService : UpdatableService<Entities.Opening, Models.Create.Opening, Models.Read.Opening, Models.Update.Opening, Models.Delete.Opening>, IOpeningService
  {
    #region Variables
    #endregion
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public Models.Read.Opening Get(int id)
    {
      ...
      return this.Source.Map<Models.Read.Opening>(opening);
    }
    ...
    /// <returns></returns>
    public IEnumerable<Models.Read.Opening> GetForCalendar(int calendarId, DateTime startOn, DateTime endOn)
    {
      ...
      var openings = (
        ...
        .Select(o => this.Source.Map<Models.Read.Opening>(o));
    ...
    /// <returns></returns>
    public Models.Read.Opening Apply(Models.Read.OpeningApplication application, Models.Read.Participant participant = null)
    {
    ...
    /// <returns></returns>
    private Models.Read.Opening Apply(Entities.Opening opening, Entities.Participant participant, params Models.Read.Answer[] answers)
    {
      var userId = this.GetUserId();
      var participantCount = opening.Participants.Count(p => p.State == Models.OpeningApplicationState.Accepted);
      ...
      var state = opening.ApplicationProcess == Models.ApplicationProcess.Review ? Models.OpeningApplicationState.Applied : Models.OpeningApplicationState.Accepted;
      ...
      PerformAction(op, Models.ActionTrigger.Apply);

      if (opening.ApplicationProcess == Models.ApplicationProcess.AutoAccept)
      {
        PerformAction(op, Models.ActionTrigger.Accept);
      }

      var result = this.Source.Map<Models.Read.Opening>(opening);
    ...
    /// <returns></returns>
    public Models.Read.Opening Unapply(Models.Read.Opening opening, params Models.Read.Participant[] participants)
    {
      ...
      if (participants == null || participants.Count() == 0)
      {
        ...
        participants = new[] { new Models.Read.Participant() { Id = participant.Id } };
      }
      ...
      {
        PerformAction(op, Models.ActionTrigger.Unapply);
      ...
      var result = this.Source.Map<Models.Read.Opening>(eopening);
      Track(eopening, result);
      return result;
    }

    private void PerformAction(Entities.OpeningParticipant openingParticipant, Models.ActionTrigger trigger)
    {

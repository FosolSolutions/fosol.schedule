...
namespace Fosol.Schedule.DAL.Services
{
  ...
  /// </summary>
  public sealed class ParticipantService : UpdatableService<Participant, Models.Create.Participant, Models.Read.Participant, Models.Update.Participant, Models.Delete.Participant>, IParticipantService
  {
    #region Variables
    #endregion
    ...
    /// <returns></returns>
    public Models.Read.Participant Get(Guid key)
    {
      var participant = this.Context.Participants.FirstOrDefault(p => p.Key == key) ?? throw new NoContentException(typeof(Models.Read.Participant));

      return this.Source.Map<Models.Read.Participant>(participant);
    }
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public Models.Read.Participant Get(int id)
    {
      return this.Source.Map<Models.Read.Participant>(this.Find((set) => set.Include(p => p.Attributes).ThenInclude(p => p.Attribute).SingleOrDefault(p => p.Id == id)));
    }
    ...
    /// <returns></returns>
    public IEnumerable<Models.Read.Participant> GetForCalendar(int calendarId, int skip = 0, int take = 20)
    {
      // TODO: verify access to calendar.
      var participants = this.Context.Participants.Where(p => p.CalendarId == calendarId).OrderBy(p => p.LastName).ThenBy(p => p.FirstName).Skip(skip).Take(take).Select(p => this.Source.Map<Models.Read.Participant>(p, null)).ToArray();
    ...
    public override Models.Update.Participant Update(Models.Update.Participant model)
    {
    ...
    public void AddAttribute(Models.Read.Participant participant, Models.Read.Attribute attribute)
    {
    }
    ...
    public IEnumerable<Claim> GetClaims(int participantId)
    {
      ...
      var email = this.Context.ContactInfo.FirstOrDefault(ci => ci.Participants.Any(pci => pci.ParticipantId == participantId) && ci.Type == Models.ContactInfoType.Email);
      ...
      var claims = new List<Claim>(new[]
      {
                ...
                new Claim(ClaimTypes.Gender, $"{participant.Gender}", typeof(Models.Gender).FullName, "CoEvent"),
